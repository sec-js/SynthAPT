...
Each task_set in the playbook represents an individual thread. When the payload is executed, it checks the task byte and executes the task_set that corresponds to that byte.
...
Each implant has a "magic" u32 that it uses to find itself in memory.
...
- If you need to make an executable a service (e.g. for psexec) use the register_service op.
"#)
}
...
fn send_message_sync(
    ...
    let api_key = get_api_key()
        .ok_or_else(|| "No API key found. Set ANTHROPIC_API_KEY env var or add api_key under [claude] in ~/.config/SynthAPT/config.toml".to_string())?;
