...
pub struct NtUnmapViewOfSection {
    pub syscall: NtSyscall,
}

unsafe impl Sync for NtUnmapViewOfSection {}

impl NtUnmapViewOfSection {
    pub const fn new() -> Self {
        NtUnmapViewOfSection {
            syscall: NtSyscall::new(0x6aa412cd),
        }
    }
    ...
    /// # Arguments
    ///
    /// * `process_handle` - Handle to the process from which to unmap the view
    /// * `base_address` - Base address of the view to unmap
    ///
    /// # Returns
    ///
    /// * `i32` - NTSTATUS code
    pub fn run(&self, process_handle: *mut c_void, base_address: *mut c_void) -> i32 {
        run_syscall!(
            self.syscall.number,
...
type NtQueryInformationFile = unsafe extern "system" fn(
...
pub struct NtDll {
    ...
    pub nt_unmap_view_of_section: NtUnmapViewOfSection,
...
impl NtDll {
    pub fn new() -> Self {
        NtDll {
            ...
            nt_unmap_view_of_section: NtUnmapViewOfSection::new(),
...
pub fn init_ntdll_funcs() {
    unsafe {
        ...
        // NtUnmapViewOfSection
        instance.ntdll.nt_unmap_view_of_section.syscall.address = ldr_function(
            instance.ntdll.module_base,
            instance.ntdll.nt_unmap_view_of_section.syscall.hash,
        );
        instance.ntdll.nt_unmap_view_of_section.syscall.number =
            get_ssn(instance.ntdll.nt_unmap_view_of_section.syscall.address);
        ...
        let nt_query_information_process_addr =
            ldr_function(instance.ntdll.module_base, 0x8cdc5dc2);
        instance.ntdll.nt_query_information_process =
            core::mem::transmute(nt_query_information_process_addr);

        // NtQueryInformationThread hash: djb2("NTQUERYINFORMATIONTHREAD") = 0xf5a0461b
        let nt_query_information_thread_addr =
            ldr_function(instance.ntdll.module_base, 0xf5a0461b);
        instance.ntdll.nt_query_information_thread =
            core::mem::transmute(nt_query_information_thread_addr);
    }
}
